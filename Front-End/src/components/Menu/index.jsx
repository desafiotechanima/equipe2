import React from 'react';
function Menu (){
  return(

<div className="w-6/6 ml-4 mt-20">  



<a href="/admin/cmaismais"> 
    <div class="shadow-lg rounded-2xl border border-blue-600 p-4 mt-2">
         <div class="flex items-center justify-between">
         <img style={{width: 60}}
           src="https://becode.com.br/wp-content/uploads/2017/02/cpplogo-460x460.png" 
           alt="img"/>    
           </div>
     </div>
</a>

<a href="/admin/c"> 
    <div class="shadow-lg rounded-2xl border border-blue-600 p-4 mt-2">
      <div class="flex items-center justify-between">
       <img style={{width: 60}}
      src="data:image/png;base64," alt="img"/> 
       </div>
    </div>
</a>   

<a href="/admin/js"> 
    <div class="shadow-lg rounded-2xl border border-blue-600 p-4 mt-2">
      <div class="flex items-center justify-between">
       <img style={{width: 60}}
      src="https://upload.wikimedia.org/wikipedia/commons/thumb/9/99/Unofficial_JavaScript_logo_2.svg/1024px-Unofficial_JavaScript_logo_2.svg.png" alt="img"/> 
       </div>
    </div>
</a>  

</div> 
  )
}
export default Menu;